...
namespace КурсоваБД
{
   ...
   class Func : Connection
   {
       static public void AddFunc(string s1)
       {
           try
           {
              ...
              // tran.Rollback();
               ...
               MessageBox.Show("Збережено");    //повідомлення про успішне виконання

           }
               //прехоплення помилки, закриття зєднання, відміна транзакції
           catch (MySql.Data.MySqlClient.MySqlException ex) { MessageBox.Show("Така книга вже зареєстрована"); tran.Rollback();
           }
      }
       ...
       static public void SelectFunc(string s1, DataGridView table)
       {
           ...
           catch (Exception ex) { MessageBox.Show(""); connn.Close(); }
       }
   }
}
